...
const data = {
  name: "frzoff",
  description: "Tắt lệnh /frz (chỉ admin)",
};
...
async function execute(interaction) {
  ...
  if (frenzyChannelId && banhMiRoleId) {
    try {
      ...
      await channel.permissionOverwrites.edit(banhMiRoleId, { SendMessages: false });
    } catch (err) {
      console.error("[frzoff] Không thể sửa permission channel:", err);
    }
  }

  await interaction.reply({ content: "⛔ Lệnh `/frz` đã được **tắt**.", ephemeral: true });
}
